...
ligados após falha de software ou rede. O Raspberry Pi e o painel não podem ser
a única camada de segurança. Também não é aceitável que um novo comando renove
indefinidamente o tempo máximo de uma bomba.

## Decisão

Cada nó de controle aplicará localmente, antes de qualquer integração MQTT:
...
3. saídas inicializadas desligadas antes de liberar o `OE`/enable do hardware;
...
- Clima essencial deve possuir fallback local próprio, separado de comandos de
  fertirrigação.
- Persistência de orçamento, motivo de reset e alarme ainda precisa ser definida
  no firmware antes de HIL.
- Nenhuma destas decisões libera fabricação ou energização da revisão A0/HOLD.
