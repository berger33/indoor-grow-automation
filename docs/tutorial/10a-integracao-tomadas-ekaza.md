...
As quatro tomadas EKAZA aparecerão como chaves remotas no painel, cada uma com
...
- telefone com aplicativo Tuya Smart ou Smart Life oficial e rede Wi-Fi 2,4 GHz;
...
10. Confirme no Home Assistant que cada entidade informa `on` e `off`; um
    dispositivo que apareça apenas como indisponível não está homologado.

Referência atual do fluxo: <https://www.home-assistant.io/integrations/tuya/>.

## Credencial entre o Grow Hub e o Home Assistant

1. crie no perfil do Home Assistant um token de longa duração exclusivo para o
   Grow Hub;
2. grave URL e token somente no gerenciador de segredos/variáveis de ambiente
   do serviço; nunca use arquivo versionado;
3. restrinja o acesso de rede ao Raspberry Pi e prefira HTTPS quando o tráfego
   deixar o próprio host;
4. reinicie o serviço e confirme uma leitura de cada entidade antes de habilitar
   comandos.

O adaptador implementado usa a API REST oficial do Home Assistant: chama
